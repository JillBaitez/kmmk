...
const IframeController = {
  ...
  async _pingIframe() {
    try {
      ...
      // Prefer a direct round-trip via iframe to avoid SW handler ambiguity
      if (window.bus._sendToIframe) {
        const ack = await Promise.race([
          window.bus._sendToIframe('startup.oiReady'),
          new Promise(resolve => setTimeout(() => resolve(null), 5000))
        ]);
        const ok = !!ack;
        if (!ok) console.warn('[OffscreenBootstrap] Iframe direct ping timed out.');
        return ok;
      }

      // Fallback: poll through SW if iframe channel is not yet wired
